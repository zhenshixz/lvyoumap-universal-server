...
const { answerIdentityCompatible, chooseBetterFailure, isTransientAnswer, parseAnswer, routeIsUsable } = require('./xhs_research_guides');
...
assert.equal(answerIdentityCompatible('黄河壶口瀑布旅游区', {
  name: '陕西黄河壶口瀑布旅游区',
  aliases: ['壶口瀑布旅游区'],
}), true, '已批准别名不应因地区前缀或景区后缀差异被误判');
...
const separateCare = answer.replace('长辈儿童：长辈建议：多使用电梯；儿童建议：可重点参观互动展区', '长辈建议：多使用电梯并适时休息 儿童建议：可重点参观互动展区');
assert.equal(parseAnswer(separateCare, target).complete, true);

const truncatedCare = answer.replace('可重点参观互动展区', '可体验互动展区，');
assert.equal(parseAnswer(truncatedCare, target).complete, false);
assert.equal(isTransientAnswer('问题分析中\n活动'), true);
assert.equal(isTransientAnswer('正在生成回答…'), true);
assert.equal(isTransientAnswer(answer), false);
