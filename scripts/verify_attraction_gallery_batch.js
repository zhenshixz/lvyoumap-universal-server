...
const policy = readJson(path.join(root, 'content', 'attraction-gallery-policy.json'));
...
for (const item of ready) {
  ...
  if (images.length < policy.minimumImages || images.length > policy.maximumImages
    || effective.length < policy.minimumImages || effective.length > policy.maximumImages) {
    errors.push(`${item.name}: 最终不是${policy.minimumImages}-${policy.maximumImages}张`);
  }
...
if (errors.length) {
  ...
  console.log(`图库构建检查通过：${ready.length}/${ready.length} 个景点均为${policy.minimumImages}-${policy.maximumImages}张唯一图片，封面一致，无拒绝图及通用搜索图。`);
}
